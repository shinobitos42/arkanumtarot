...
import { Search, Mic, Send, Paperclip, MoreVertical, Play, Pause, CheckCheck, Menu, Square, Loader2, User, Star } from "lucide-react";
import api from "../services/api";

// COMPONENTE: Player de Áudio Customizado (Estilo WhatsApp do Arcanum)
...
// COMPONENTE PRINCIPAL: Tela de Mensagens
export default function Mensagens({ customStyle }) {
  ...
  useEffect(() => {
    ...
    // Reseta o estado da avaliação ao trocar de chat
    setNotaDada(0);
    ...
    const buscarMensagens = async () => {
      try {
        ...
        // Também atualiza o status do chat atual na lista (caso o tarólogo tenha encerrado)
  ...
  // FUNÇÃO: Tarólogo Encerrar a Leitura
  const encerrarSessao = async () => {
    if (!window.confirm("Tem certeza que deseja encerrar esta leitura? O chat será fechado para novas mensagens.")) return;
    
    try {
      await api.patch(`tiragens/sessoes/${chatAtivoId}/`, {
        status_sessao: 'finalizada'
      });
      
      setChats(prevChats => prevChats.map(chat => 
        chat.id === chatAtivoId ? { ...chat, status_sessao: 'finalizada' } : chat
      ));
      
    } catch (error) {
      console.error("Erro ao encerrar sessão:", error);
      alert("Houve um erro ao tentar encerrar a sessão.");
    }
  };

  // FUNÇÃO: Consulente Enviar Avaliação
  const enviarAvaliacao = async () => {
    ...
    try {
      ...
      // Atualiza o estado local para esconder a caixa imediatamente
      setChats(prevChats => prevChats.map(chat => 
        chat.id === chatAtivoId ? { ...chat, nota: notaDada } : chat
      ));
  ...
  const handleEnviarTexto = async (e) => {
    ...
    try {
      await api.post('tiragens/mensagens/', {
        sessao: chatAtivoId,
        texto: textoEnvio,
        tipo: 'texto'
      });
  ...
  const iniciarGravacao = async () => {
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
          const response = await api.post('tiragens/mensagens/', formData, {
      ...
      alert("Permita o acesso ao microfone no seu navegador.");
    }
  };
        ...
        <div style={styles.sidebar}>
          ...
          <div style={styles.contactList}>
            {loadingChats ? (
              ...
              <p style={{ color: '#786C63', textAlign: 'center', padding: '40px', fontSize: '14px' }}>
              ...
              chats.map((chat) => {
                ...
                return (
                  <div 
                    key={chat.id} 
                    onClick={() => setChatAtivoId(chat.id)}
                    style={{
                      ...styles.contactItem,
                      ...(chatAtivoId === chat.id ? styles.contactItemAtivo : {})
                    }}
                  >
          ...
          <>
            <div style={styles.chatHeader}>
              <div style={styles.chatHeaderLeft}>
                <button onClick={() => setMostrarSidebar(!mostrarSidebar)} style={styles.menuToggleBtn}>
              ...
              <div style={styles.chatHeaderRight}>
                {role === "TAROLOGO" && chatAtivo.status_sessao === 'ao_vivo' && (
                  <button onClick={encerrarSessao} style={styles.btnEncerrar}>
                    Encerrar Leitura
                  </button>
                )}
                <button style={styles.iconBtn}><Search size={18} color="#A89C92" /></button>
            ...
            <div style={styles.chatMessages}>
              {loadingMensagens ? (
                ...
                <div style={{ textAlign: 'center', marginTop: '60px' }}>
                   <p style={{ color: '#A89C92', fontStyle: 'italic' }}>
                     {chatAtivo.status_sessao === 'aguardando_guia' 
                       ? "Aguarde um guia aceitar seu pedido para enviar mensagens." 
                       : "A sessão foi aberta. A energia está pronta para fluir."}
                   </p>
                </div>
              ) : (
                mensagens.map((msg) => {
                  ...
                  return (
                    ...
                    >
                      <div style={{
                        ...
                        ...(isMyMessage ? styles.messageUser : styles.messageGuia)
                      }}>
                        
                        {msg.tipo === "audio" ? (
                           <CustomAudioPlayer audioUrl={msg.arquivo_audio} />
                        ) : (
                          <p style={styles.messageText}>{msg.texto}</p>
                        )}
              ...
              {/* AQUI ESTÁ A CORREÇÃO MÁGICA: !chatAtivo.nota */}
                  ...
                  {avaliacaoEnviada ? (
                    ...
                    <>
                      ...
                      <div style={styles.starsContainer}>
                        {[1, 2, 3, 4, 5].map((star) => (
                          <Star 
                            ...
                            color={star <= (hoverNota || notaDada) ? "#D4AF37" : "#3A322C"} 
                            fill={star <= (hoverNota || notaDada) ? "#D4AF37" : "transparent"} 
                      ...
                      <button 
                        ...
                        disabled={notaDada === 0}
                        style={{...styles.btnAvaliar, opacity: notaDada === 0 ? 0.5 : 1}}
                      >
                        Enviar Avaliação
                      </button>
                    </>
              ...
              <div style={styles.chatInputWrapper}>
                ...
                <form onSubmit={handleEnviarTexto} style={styles.inputForm} autoComplete="off">
                  <input 
                    type="text" 
                    placeholder={
                      chatAtivo.status_sessao === 'finalizada' ? "Sessão encerrada." :
                      isRecording ? "Gravando voz... Clique no botão vermelho para enviar." : 
                      "Digite sua mensagem..."
                    } 
                    style={{ ...styles.textInput, color: isRecording ? '#ef4444' : '#EAE0C8', fontWeight: isRecording ? '600' : '300' }}
                    value={novaMensagem}
                  ...
                  {novaMensagem.trim() ? (
                    <button type="submit" style={styles.sendBtn} disabled={chatAtivo.status_sessao === 'finalizada'}>
                      <Send size={16} color="#151312" />
                    </button>
                  ) : (
                    <button 
                      type="button" 
                      onClick={toggleGravacao}
                      disabled={chatAtivo.status_sessao === 'aguardando_guia' || chatAtivo.status_sessao === 'finalizada'}
                      style={{
                        ...styles.micBtn,
                        backgroundColor: isRecording ? "rgba(239, 68, 68, 0.15)" : "transparent",
                        borderColor: isRecording ? "#ef4444" : "#3A322C",
                        opacity: (chatAtivo.status_sessao === 'aguardando_guia' || chatAtivo.status_sessao === 'finalizada') ? 0.5 : 1
                      }}
                    >
...
const styles = {
  ...
  sidebarTitle: { color: "#FDFBF7", fontFamily: "'Playfair Display', serif", fontSize: "20px", marginBottom: "20px", fontWeight: "normal" },
  ...
  contactTime: { color: "#786C63", fontSize: "12px" },
  ...
  chatHeaderAvatar: { width: "48px", height: "48px", borderRadius: "50%", objectFit: "cover", border: "1px solid #3A322C", backgroundColor: "#2A2420" },
  ...
  // AQUI ADICIONAMOS O WORD-BREAK PARA O TEXTO DO CHAT TAMBÉM NÃO VAZAR
  ...
  customAudioBtn: { width: '40px', height: '40px', borderRadius: '50%', backgroundColor: '#D4AF37', display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'none', cursor: 'pointer', flexShrink: 0, transition: 'transform 0.1s ease', ':active': { transform: 'scale(0.95)' } },
  ...
  avaliacaoCard: { alignSelf: 'center', backgroundColor: '#1A1715', border: '1px solid #D4AF37', borderRadius: '12px', padding: '32px', display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', maxWidth: '340px', marginTop: '20px', boxShadow: '0 10px 30px rgba(212, 175, 55, 0.05)' },
  ...
  btnAvaliar: { width: '100%', padding: '14px', backgroundColor: '#D4AF37', color: '#151312', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '1px', cursor: 'pointer', transition: 'all 0.2s' },
