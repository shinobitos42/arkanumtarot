...
import { 
  Wallet, Users, Layers, Zap, Search, Bell, Moon, 
  MessageCircle, Settings, Check, ChevronDown, Lock, Star, LogOut, Clock, Sparkles, TrendingUp, Loader2
} from "lucide-react";
...
export default function PainelTarologo() {
  ...
  useEffect(() => {
    const buscarDadosDoPainel = async () => {
      try {
        const resPerfil = await api.get('users/me/');
        setPerfil(resPerfil.data);
        
        const saldoFormatado = resPerfil.data.saldo_disponivel 
          ? parseFloat(resPerfil.data.saldo_disponivel).toFixed(2).replace('.', ',') 
          : "0,00";
        ...
        setEstatisticas({
          ...
          nota: resPerfil.data.nota_media || "5.0"
        });
  ...
  const extrairContextoLimpo = (textoContexto) => {
    ...
    // Regex aprimorado: limpa a tag inteira e quaisquer espaços ou quebras de linha inúteis logo após ela
    ...
    <div style={styles.appContainer}>
      ...
      <aside style={styles.sidebar}>
        <div style={styles.logoContainer}>
          ...
          <h2 style={styles.logoText}>Arcanum Pro</h2>
        </div>
              ...
              <div style={styles.filaGrid}>
                {filaExpressa.map((pedido) => (
                  <div key={pedido.id} style={styles.pedidoCard}>
                    ...
                    {/* Dúvida e Contexto com Design Elegante */}
          ...
          <>
            <div style={styles.statsGrid}>
              <div style={styles.statCard}>
                ...
                <h3 style={styles.statValue}>R$ {estatisticas.saldo}</h3>
                <button style={styles.btnSacar}>Solicitar Saque (Pix)</button>
              </div>
              ...
              <div style={styles.statCard}>
                ...
                <h3 style={styles.statValue}>{estatisticas.tiragens}</h3>
              ...
              <div style={styles.statCard}>
                ...
                <h3 style={styles.statValue}>{estatisticas.consulentes}</h3>
                <p style={styles.statDesc}>Nota média: {estatisticas.nota} <Star size={10} color="#D4AF37" fill="#D4AF37" style={{display: 'inline', marginLeft:'2px'}}/></p>
              </div>
            </div>

            <div style={styles.historyCard}>
               <h3 style={{color: '#FDFBF7', fontFamily: "'Playfair Display', serif", fontSize: '18px', marginBottom: '16px'}}>
                 Últimos Lançamentos
               </h3>
               <div style={{display: 'flex', justifyContent: 'center', padding: '40px', color: '#786C63', fontSize: '14px', border: '1px dashed #2A2420', borderRadius: '8px'}}>
                 Seu histórico de ganhos aparecerá aqui após o primeiro saque.
               </div>
            </div>
          </>
        )}
          ...
          <div style={styles.profileContainer}>
            <div style={styles.cardSettings}>
              ...
              <form style={styles.form}>
                ...
                <hr style={{ border: 'none', borderTop: '1px solid #2A2420', margin: '16px 0' }} />

                <h3 style={styles.sectionTitle}>Vitrine do Guia (Público)</h3>

                <div style={styles.formGroup}>
                  <label style={styles.label}>Especialidade Principal (Ex: Astrologia & Tarot de Thoth)</label>
                  <input 
                    type="text" 
                ...
                <div style={styles.formGroup}>
                  <label style={styles.label}>Biografia e Metodologia de Leitura</label>
                  <textarea 
                    defaultValue={perfil.biografia}
                    placeholder="Conte aos consulentes como funciona a sua leitura, suas regras e seu acolhimento..."
                    style={styles.textarea}
                  ></textarea>
                </div>

                <button type="button" style={styles.btnSalvar}>Atualizar Vitrine</button>
              </form>
            </div>
          </div>
        )}
...
const styles = {
  ...
  sidebar: { width: "260px", backgroundColor: "#151312", borderRight: "1px solid #2A2420", padding: "32px 20px" },
  ...
  statCard: { backgroundColor: "#1A1715", border: "1px solid #2A2420", borderRadius: "12px", padding: "24px" },
  statHeader: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" },
  statTitle: { color: "#A89C92", fontSize: "14px" },
  ...
  btnSacar: { width: "100%", padding: "10px", marginTop: "16px", backgroundColor: "transparent", border: "1px solid #D4AF37", color: "#D4AF37", borderRadius: "6px", fontSize: "12px", fontWeight: "600", textTransform: "uppercase", cursor: "pointer" },

  historyCard: { backgroundColor: "#151312", border: "1px solid #2A2420", borderRadius: "12px", padding: "24px" },
  ...
  // NOVOS ESTILOS DO CARD DA FILA EXPRESSA
  ...
  btnSalvar: { padding: "14px", backgroundColor: "#D4AF37", color: "#151312", border: "none", borderRadius: "8px", fontSize: "14px", fontWeight: "700", cursor: "pointer", marginTop: "16px" }
};